...
- **Google Drive no Thunar**: mesma stack do GNOME (GVfs + GNOME Online Accounts), sem precisar do Nautilus. Adicione a conta em "Contas Online" (`gnome-control-center online-accounts`), depois a pasta **Google Drive** aparece na barra lateral do Thunar e **só monta quando você clica** (lazy-mount, nada automático no boot). O token OAuth fica no `gnome-keyring` — se ele pedir a senha do chaveiro na primeira montagem, cadastre uma.
- **Alternativa mais leve ao GOA** (zero daemon residente): `rclone` — `rclone config` (remote tipo `gdrive`), montagem manual com `rclone mount gdrive: ~/GDrive --daemon` e desmonte com `fusermount -u ~/GDrive`. Pode virar ação personalizada do Thunar. Só vale a pena se a UX do GOA pesar no seu gosto; o custo do GOA é só o `goa-daemon` (~15MB RAM).
